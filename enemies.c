...
int stat_kstrl(STATEK* sta, KAPSULA* kaps)
{
    ...
    if (sta->giwera == 0 || sta->giwera == 1) {
        if (sta->x > kaps->x - 30 && sta->x < kaps->x + 60)
            zwrot = 1;
    } else if (sta->giwera == 2) {
        for (i = 0; i < 3; i++) {
            if (sta->x > sta->poc_x[i] - 30 && sta->x < sta->poc_x[i] + 60)
                zwrot = 1;
        }
    } else {
...
void stat_wykonaj(void)
{
    ...
    if ((licznik++) > 2)
        licznik = 0;
    for (i = 0; i < 5; i++)
        statek_wykonaj(&statek[i], licznik);
}
